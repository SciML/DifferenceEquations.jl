...
fd_gradient_sim!(vec(copy(sim_fd_s.A)), sim_fd_s.B, sim_fd_s.C,
    sim_fd_s.u0, sim_fd_s.noise, p_sim_fd_small.N)
fd_gradient_sim!(vec(copy(sim_fd_l.A)), sim_fd_l.B, sim_fd_l.C,
...
SIM_FD["gradient"]["small_static"] = let
    ...
    function _sim_scalar_static(A_vec, B_s, C_s, noise_s,
        ...
        prob = LinearStateSpaceProblem(A_d, B_d, u0_d, (0, length(noise_s));
            C = C_d, noise = noise_s)
        sol = solve(prob, DirectIteration())
        return sum(sol.u[end])
    end
