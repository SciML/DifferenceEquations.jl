...
function make_kalman_benchmark(p; seed = 42)
    ...
    return (; A, B, C, R, mu_0, Sigma_0, y, prob, cache,
        dprob, dcache, dA, dB, dC, dmu_0, dSigma_0, dR, dy)
end
...
# =============================================================================
# Forward wrapper (returns matrices from cache for tangent validation)
# =============================================================================
...
function raw_kalman!(prob, cache)
    ws = StateSpaceWorkspace(prob, KalmanFilter(), cache)
    return solve!(ws).logpdf
end

# Warmup
raw_kalman!(kf_s.prob, kf_s.cache)
raw_kalman!(kf_l.prob, kf_l.cache)

KALMAN_ENZYME["raw"]["small_mutable"] = @benchmarkable raw_kalman!(
    $(kf_s.prob), $(kf_s.cache))
KALMAN_ENZYME["raw"]["large_mutable"] = @benchmarkable raw_kalman!(
    $(kf_l.prob), $(kf_l.cache))

# =============================================================================
# Forward mode AD — perturb A[1,1], return computed matrices
# =============================================================================

function forward_kalman_bench!(A, B, C, mu_0, Sigma_0, R, y, prob, cache,
        dA, dB, dC, dmu_0, dSigma_0, dR, dy, dprob, dcache)
    # Zero all shadows
    make_zero!(dprob)
    make_zero!(dcache)
...
function reverse_kalman_bench!(A, B, C, mu_0, Sigma_0, R, y, prob, cache,
        dA, dB, dC, dmu_0, dSigma_0, dR, dy, dprob, dcache)
    # Zero all shadows
    make_zero!(dprob)
    make_zero!(dcache)
