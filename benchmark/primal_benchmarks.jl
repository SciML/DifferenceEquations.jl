...
function joint_likelihood_2(A_0, A_1, A_2, B, C_0, C_1, C_2, u0, noise, observables, D; kwargs...)
    prob = QuadraticStateSpaceProblem(
        A_0, A_1, A_2, B, u0, (0, size(observables, 2)); C_0, C_1,
...
const D_2_FVGQ = abs2.(ones(6) * 1.0e-3)
...
println()
println("=" ^ 70)
println("QUADRATIC MODEL — RBC (2×2, T=$(size(observables_2_rbc, 2)))")
println("=" ^ 70)
...
println()
println("=" ^ 70)
println("QUADRATIC MODEL — FVGQ ($(size(A_1_FVGQ,1))×$(size(A_1_FVGQ,2)), T=$(size(observables_2_FVGQ, 2)))")
println("=" ^ 70)
