...
@testset "ForwardDiff - Kalman Filter (mutable)" begin
    @testset "primal sanity" begin
        loglik_val = kalman_loglik_fd(
            A_kf_fd, B_kf_fd, C_kf_fd,
    ...
    @testset "gradient w.r.t. A" begin
        f = a_vec -> kalman_loglik_fd(
            reshape(a_vec, N_kf_fd, N_kf_fd),
    ...
    @testset "gradient w.r.t. B" begin
        f = b_vec -> kalman_loglik_fd(
            A_kf_fd, reshape(b_vec, N_kf_fd, K_kf_fd),
...
function kalman_loglik_fd_static(
        A_vec, B, C, mu_0, Sigma_0, R, y,
        ::Val{N}, ::Val{M}, ::Val{K}
    ) where {N, M, K}
    T_el = eltype(A_vec)
...
@testset "ForwardDiff - Kalman Filter (static)" begin
    @testset "gradient w.r.t. A" begin
        f = a_vec -> kalman_loglik_fd_static(
            a_vec, B_kf_fd_s, C_kf_fd_s,
