...
@testset "mean_z test" begin
    @test z_sum(A_rbc, B_rbc, C_rbc, u0_rbc, noise_rbc, observables_rbc, D_rbc) ≈
        -0.09008162336682057
    gradient(
        (args...) -> z_sum(args..., observables_rbc, D_rbc), A_rbc, B_rbc, C_rbc,
        u0_rbc, noise_rbc
    )
    test_rrule(
...
@testset "u test" begin
    @test u_sum(A_rbc, B_rbc, C_rbc, u0_rbc, noise_rbc, observables_rbc, D_rbc) ≈
        -0.08780558376240931
    gradient(
        (args...) -> u_sum(args..., observables_rbc, D_rbc), A_rbc, B_rbc, C_rbc,
        u0_rbc, noise_rbc
    )
    test_rrule(
        Zygote.ZygoteRuleConfig(),
...
@testset "W test" begin
    gradient(
        (args...) -> W_sum(args..., observables_rbc, D_rbc), A_rbc, B_rbc, C_rbc,
        u0_rbc, noise_rbc
    )
    test_rrule(
        Zygote.ZygoteRuleConfig(),
...
function no_observables_sum(A, B, C, u0, noise; kwargs...)
    problem = LinearStateSpaceProblem(
        A, B, u0, (0, size(noise_rbc, 2)); C, noise,
        kwargs...
    )
    sol = solve(problem)
    return sol.W[1, 2] + sol.W[1, 4] + sol.z[2][2]
...
@testset "no observables gradient" begin
    @test no_observables_sum(A_rbc, B_rbc, C_rbc, u0_rbc, noise_rbc) ≈
        -0.08892781958364693
    gradient(
        (args...) -> no_observables_sum(args...), A_rbc, B_rbc, C_rbc,
        u0_rbc, noise_rbc
    )
    test_rrule(
        Zygote.ZygoteRuleConfig(),
...
@testset "no noise" begin
    ...
    gradient(
        (args...) -> no_noise(args...), A_rbc, C_rbc,
        u_nonzero
    )
    test_rrule(
        Zygote.ZygoteRuleConfig(),
...
@testset "no observation equation" begin
    ...
    gradient(
        (args...) -> no_observation_equation(args...), A_rbc,
        u_nonzero
    )
    test_rrule(
        Zygote.ZygoteRuleConfig(),
...
@testset "no observation equation" begin
    ...
    gradient(
        (args...) -> no_observation_equation_noise(args...), A_rbc, B_rbc,
        u_nonzero
    )
    test_rrule(
        Zygote.ZygoteRuleConfig(),
...
@testset "last observable with noise, no observable noise" begin
    ...
    test_rrule(
        Zygote.ZygoteRuleConfig(),
        (u_nonzero) -> last_observable_pass_noise(
            A_rbc, B_rbc, C_rbc, u_nonzero,
            noise
        ),
        u_nonzero;
        rrule_f = rrule_via_ad,
